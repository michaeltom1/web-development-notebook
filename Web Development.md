...
The reason why we need a paragraph tag is because if you just had plain text in your HTML document.

and open it in a web browser

The text they run into each other onto the same line and you can't actually tell where one paragraph begins and where the other ends

On the other hand, if we had created two paragraph elements for these two lines, then when we run the code we will see them being separated with a line (space) in between to be able to distinguish the different paragraphs.

And this is also really important for people who are using screen readers,like blind people. what the screen reader will do is allow them see(know) the beginning of each paragraph element and it allows the blind user to be able to skip to the next paragraph easily and be able to see what's going

## Lorem Ipsum

Lorem Ipsum is a placeholder text.

and it's something that you'll come across quite a lot, especially in web design

it is use so that we don't end up with something like this, where we're trying to simulate content in a paragraph

but then we're just writing content, content, content or This is a paragraph, This is a paragraph, This is a paragraph

And the reason why we don't want to do this is because these are all the same words. The sentence has the same length.

it doesn't look like what a natural block of text would look like and we don't have the time or energy to actually create the content just for testing and web design.

so that's why we use Lorem Ip sum

the idea is to have very natural looking paragraphs that mimic pretty much what a regular English article would look like

Lorem Ipsum apparently comes form latin classical literature by a famous author called Cicero

it's over 2000 years old

and it has been used in the print industry for many, many years, ever since the 1500s
...
non-void element such as the paragraph element or the heading elements are not void because they have content in between their opening and closing tags

### Features of void elements

the tag itself looks very different

`<hr />`

this is a horizontal rule element and it

starts with an angle bracket `<`

contain the tag name `hr`

and normally by convention has a space ` ` after the tag name

it contains a forward slash `/` just before the end of the tag

end with and angle bracket `>`

> Note: The void element is not to be confused with the closing tag of a non-void element in a closing tag, the forward slash is just after the opening angle bracket of the closing tag in front of the tag name `</p >` whereas in the void elements the forward slash is actually just before the end of the tag `<hr />`
...
The break element is another void element and it looks like this

`<br / >`

it is used to separate things onto separate lines

like things ment to be in the same paragraphs but different lines
...
Tip
diffchecker.com is used to check the difference between to text

### DOs and DON't

do not use a `<br />` tag to create space between two paragraphs

instead create two separate paragraphs element for the two paragraphs

because this is not very good for accessibility
...
with or without the forward slash
these are all valid options


As of HTML5, the latest version of HTML 
the forward slash is ignored 

**Recommendation**

Keep the forward slash 
And the reason is because when you read your code, you as the human, it will be much easier for you to see that this is a void element and it doesn't need a closing tag so that you don't end up being confused and you can identify the void elements and the non-void elements easily and distinguish them from each other 
